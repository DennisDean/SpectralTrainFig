classdef SpectralTrainClass
    ...
    % Last updated: April 13, 2016 by Sara
    %
        ...
        function obj = computeSpectralTrain(varargin)
            ...
            if START_ANALYSIS_FOR_EACH_FILE == 1
                ...
                for f = processStart:processFiles
                    ...
                    %% remove ECG artifact
                    if obj.denoiseEcg
                        fprintf('\t%s\n', 'Correcting ECG contamination');
                        edfObj2 = BlockEdfLoadClass(edfFn);
                        edfObj2.numCompToLoad = 3;      % Don't return object
                        edfObj2 = edfObj2.blockEdfLoad;  % Load data
                        lab=edfObj2.signal_labels;
                        ecgindex=unique(find(strcmpi(lab,obj.ecgName)));
                        if isempty(ecgindex)
                            % try to look for something similar
                            ecgindex=unique([find(~cellfun(@isempty,strfind(lab,obj.ecgName))) ...
                                find(~cellfun(@isempty,strfind(lab,upper(obj.ecgName)))) ...
                                find(~cellfun(@isempty,strfind(lab,lower(obj.ecgName))))]);
                            if isempty(ecgindex)
                                error(['There are no channels named or containing ', ...
                                    cell2mat(obj.ecgName)]);
                            else
                                ecgindex=ecgindex(1);
                                warning(['There are no channels named ', ...
                                cell2mat(obj.ecgName) '. The channel ' lab{ecgindex} ' will be used instead'])
                            end
                        else
                        ...
                        for s = 1:processNsignals
                            ...
                            if PLOT_ARTIFACT_SUMMARY == 1
                                ...
                                ylabel(sprintf('Density (%s^2)',analysisEegUnits),...
                                    'FontWeight','bold','FontSize',fontSize);
                                %set(gca,'yscale', 'log');
                                ylim([0 max(log10(deltaSpectrum))+1])
                                v = axis;
                                ...
                                %set(gca,'yscale', 'log');
                                ylim([0 max(log10(betaSpectrum))+1])
                                v = axis;
                            ...
                            if PLOT_SPECTRAL_SUMMARY == 1;
                                ...
                                imagesc(flipud(pxxEpoch(fIdx,:))*scalingFactorForEffect, [-10 25]);
                                colormap(map);
                                colorbar;
    ...
    methods(Static)
        ...
        function CloseChildrenFigures
            ...
            indexes=[];
            for jj=1:length(hands)
                if ~isempty(hands(jj).Number)
                    close (hands(jj))
                end
            end
